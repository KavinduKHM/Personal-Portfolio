              ...
              <NavLink key={l.to} to={l.to} end={l.end} className="nav-link">
                {({ isActive }) => (
                  <span className="nav-link-inner">
                    {isActive && (
                      <motion.span layoutId="activeNavPill" className="nav-active-pill" style={{ position: "absolute", inset: 0, borderRadius: 9999, zIndex: 1 }} />
                    )}
                    <span style={{ position: "relative", zIndex: 2, display: "inline-block" }} className={isActive ? 'nav-link-text active' : 'nav-link-text'}>{l.label}</span>
                  </span>
                )}
              </NavLink>
            ))}
